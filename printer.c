	...
	if (! IsCancelPack)
	{
		// 溫度提高
			...
			for(Line = 0;Line <Packer.PgMode[USB.PgMode].Length ; Line++)
			{
				if(IsPrint)
				{
					if (Line == Packer.PgMode[USB.PgMode].Left - 55)
					{
						u14pb |= 0x40;				//印字頭上升
						_u14pb(u14pb);
						Delay(20);
					...
					else if (Line == Packer.PgMode[USB.PgMode].Right)
					{
//						u14B = u14PC_PresureStop;	//不確定是否有功能需要測試
						u14pb &= 0xBF;				//印字頭下降
						_u14pb(u14pb);				//後來加上去確定有用
						Delay(1000);				//原始20
						Carbon(0);
						Delay(10);
					}			
				...
				else
				{
					...
					if(Line < 200)
						MotorTime --;
					else if(Line > (Packer.PgMode[USB.PgMode].Length - 200))
						MotorTime ++;
				}
			}
	}	
...
//	Packer.Cpu1.CarbonPWM =0x10;
	...
	Packer.PgMode[0].Left 	 = 120;
	Packer.PgMode[0].Right 	 = 560;
	Packer.PgMode[0].Length  = 585;	
	Packer.PgMode[0].SpacePg = 4;

	//9cm		
	Packer.PgMode[1].Left  	 = 80; //60
	Packer.PgMode[1].Right 	 = 600;//580
	Packer.PgMode[1].Length  = 750;//750 8.7=720
	Packer.PgMode[1].SpacePg = 3;
	
	//10cm	
	Packer.PgMode[2].Left 	 = 300;
	Packer.PgMode[2].Right 	 = 820;//820
	Packer.PgMode[2].Length  = 830;
	Packer.PgMode[2].SpacePg = 3;
