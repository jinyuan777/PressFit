...
xdata  unsigned char mem[55],men[55];	
unsigned int Addr=0x0000;
unsigned char ReadNumbers[10];
void Powder(unsigned char type,unsigned char pwm){
	PCA0CPH0=pwm;
	if (type == MotorStop)
	{
...
/*
	...
	*/
	...
	while(1){
		if(Packer.Cpu1.Command_USB == Cpu1Command_Cut){
//			PresureDown();
/*			EA = 0;			
			Addr =0x0000;
			for(i=0;i<50;i++){
...
void  Page_Write (unsigned int  PageAddress)
{
   unsigned char  EA_Save,ii;             // Used to save state of global interrupt enable
   unsigned char  xdata *pread;    // Read Pointer
   unsigned int  x;                  // Counter for 0-512 bytes
   pread =  (unsigned char xdata *)(TempStorage);
   EA_Save  =  EA;            // Save EA
   EA =  0;                   // Turn off interrupts       
   for(x = PageAddress;  x<PageAddress + FLASH_PAGE_SIZE;   x++) // Write 512 bytes
   {
   	//	ii = x/55;
	//	jj = x%55; 
...
void  Receive_Setup (void)
{
   ...
   if (Buffer[0]  == READ_MSG)         // Check See if Read File Setup
   {
      ...
      ReadIndex   =  PageIndices[0];
   		...
   		M_State = ST_IDLE_DEV;
		//_OnePack();
		Packer.Cpu1.Command_USB = Cpu1Command_Cut;
  	    //Buffer[0]   =  0xFF;
...
{
 //  unsigned char i,ReadNumbers[]={0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
